...
SEQ_LEN = 33 # Sequence length in characters
DIM = 256 # Model dimensionality. This is fairly slow and overfits, even on
...
#         lines[i][j]=charmap[lines[i][j]]

# lines=to_categorical(lines)
...
try:
    ...
    with open("../SoftOutputs.pickle", 'rb') as f:
        lines = pickle.load(f)
...
real_inputs_discrete = tf.placeholder(tf.float32, shape=[BATCH_SIZE, SEQ_LEN,len(charmap)])
real_inputs = real_inputs_discrete   #tf.one_hot(real_inputs_discrete, len(charmap))
...
true_char_ngram_lms = [language_helpers.NgramLanguageModel(i+1, linescopy[10*BATCH_SIZE:], tokenize=False) for i in range(4)]
validation_char_ngram_lms = [language_helpers.NgramLanguageModel(i+1, linescopy[:10*BATCH_SIZE], tokenize=False) for i in range(4)]
for i in range(4):
    print ("validation set JSD for n={}: {}".format(i+1, true_char_ngram_lms[i].js_with(validation_char_ngram_lms[i])))
true_char_ngram_lms = [language_helpers.NgramLanguageModel(i+1, linescopy, tokenize=False) for i in range(4)]
...
with tf.Session() as session:
    ...
    for iteration in range(ITERS):
        ...
        if iteration % 10 == 9:
            saver.save(session, './model/my_test_model')
            samples = []
            for i in range(10):
